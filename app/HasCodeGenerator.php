...
trait HasCodeGenerator
{
    ...
    public static function generateCodeWithDateByCompany(
        string $tableName,
        string $columnName,
        string $prefix,
        int $digits,
        ...
        return DB::transaction(function () use ($companyId, $tableName, $columnName, $prefix, $digits, $separator) {
            $now = now();
            $datePart = $now->format('Ymd'); // Untuk diselipkan di tengah kode
            $yearPart = $now->format('Y');   // Untuk mencari semua data di tahun yang sama

            // 1. Cek Company (pakai exists agar lebih cepat dan tidak me-lock tabel)
            $companyExists = DB::table('companies')
                ->where('id', $companyId)
                ->exists();

            if (! $companyExists) {
                throw new \Exception('Company data is not found or disabled.');
            }

            // 2. Cari baris yang ter-insert paling terakhir di tahun ini
            $lastRecord = DB::table($tableName)
                ->where('company_id', $companyId)
                // Mencari awalan seperti: PR-2026%
                ->where($columnName, 'like', $prefix.$separator.$yearPart.'%')
                // Ubah orderBy ini ke 'id' agar yang diambil benar-benar data terakhir yang dibuat
                ->orderBy('id', 'desc')
                ->lockForUpdate()
                ->first();
